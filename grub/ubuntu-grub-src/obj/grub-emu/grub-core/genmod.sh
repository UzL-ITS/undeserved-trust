...
if test x0 != x1; then
    ...
    if test -n "$deps"; then
	gcc-8 -no-pie -no-pie -Wl,--build-id=none -ffreestanding -nostdlib -o $tmpfile2 $t1 $t2 $tmpfile -Wl,-r,-d
    else
	gcc-8 -no-pie -no-pie -Wl,--build-id=none -ffreestanding -nostdlib -o $tmpfile2 $t1 $tmpfile -Wl,-r,-d
    fi
    rm -f $t1 $t2 $tmpfile
    mv $tmpfile2 $tmpfile
...
if test xemu != xemu; then
    ./build-grub-module-verifier $tmpfile x86_64 emu
fi
mv $tmpfile $outfile

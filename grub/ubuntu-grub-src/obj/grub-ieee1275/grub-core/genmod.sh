...
if test x0 != x1; then
    ...
    if test -n "$deps"; then
	gcc-8 -no-pie -m32 -Wl,-melf_i386 -no-pie -Wl,--build-id=none -ffreestanding -nostdlib -o $tmpfile2 $t1 $t2 $tmpfile -Wl,-r,-d
    else
	gcc-8 -no-pie -m32 -Wl,-melf_i386 -no-pie -Wl,--build-id=none -ffreestanding -nostdlib -o $tmpfile2 $t1 $tmpfile -Wl,-r,-d
    fi
    rm -f $t1 $t2 $tmpfile
    mv $tmpfile2 $tmpfile
...
if test xieee1275 != xemu; then
    ./build-grub-module-verifier $tmpfile i386 ieee1275
fi
mv $tmpfile $outfile
